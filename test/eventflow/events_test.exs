defmodule Eventflow.EventsTest do
  ...
  describe "events" do
    ...
    test "create_event/1 with valid data creates a event" do
      valid_attrs = %{capacity: 42, datetime: ~N[2024-07-28 13:40:00], description: "some description", duration: 42, fee: "120.5", is_offline: true, location: "some location", published_at: ~N[2024-07-28 13:40:00], rsvp: true, status: "some status", tags: "some tags", thumbnail: "some thumbnail", title: "some title"}

      assert {:ok, %Event{} = event} = Events.create_event(valid_attrs)
      assert event.capacity == 42
      assert event.datetime == ~N[2024-07-28 13:40:00]
      assert event.description == "some description"
      assert event.duration == 42
      assert event.fee == Decimal.new("120.5")
      assert event.is_offline == true
      assert event.location == "some location"
      assert event.published_at == ~N[2024-07-28 13:40:00]
